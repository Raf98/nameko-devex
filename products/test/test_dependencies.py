...
def test_delete(create_product, redis_client, storage):

    created_product = create_product(id=1, title='LZ 127', in_stock=10)
    storage.delete(created_product['id'])
    assert redis_client.hlen('products:LZ127') == 0
